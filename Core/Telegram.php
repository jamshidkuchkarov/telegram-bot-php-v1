...
class Telegram
{
    ...
    /**
     * Create a Core instance from the bot token
    ...
    /// A method for responding http to Core.

    /**
     * \return the HTTP 200 to Core.
     */
     ...
     * <td>String</td>
     * <td>Optional</td>
     * <td>Send <em>Markdown</em>, if you want Core apps to show bold, italic and inline URLs in your bot's message. For the moment, only Core for Android supports this.</td>
     * </tr>
     * <tr>
     ...
     * <td>Yes</td>
     * <td>Photo to send. Pass a file_id as String to send a photo that exists on the Core servers (recommended), pass an HTTP URL as a String for Core to get a photo from the Internet, or upload a new photo using multipart/form-data.</td>
     * </tr>
     * <tr>
     * <td>caption</td>
     * <td>String</td>
    ...
    /**
     * Use this method to send audio files, if you want Core clients to display them in the music player. Your audio must be in the .mp3 format. On success, the sent Message is returned. Bots can currently send audio files of up to 50 MB in size, this limit may be changed in the future.
     ...
     * <td><a href="https://core.telegram.org/bots/api#inputfile">InputFile</a> or String</td>
     * <td>Yes</td>
     * <td>Audio file to send. Pass a file_id as String to send an audio file that exists on the Core servers (recommended), pass an HTTP URL as a String for Core to get an audio file from the Internet, or upload a new one using <strong>multipart/form-data</strong>.</td>
     * </tr>
     * <tr>
     * <td>duration</td>
     * <td>Integer</td>
     ...
     * <td>Yes</td>
     * <td>File to send. Pass a file_id as String to send a file that exists on the Core servers (recommended), pass an HTTP URL as a String for Core to get a file from the Internet, or upload a new one using <strong>multipart/form-data</strong>.</td>
     * </tr>
     * <tr>
     * <td>caption</td>
     * <td>String</td>
     ...
     * <td>Yes</td>
     * <td>Animation to send. Pass a file_id as String to send an animation that exists on the Core servers (recommended), pass an HTTP URL as a String for Core to get an animation from the Internet, or upload a new animation using multipart/form-data. <a href="#sending-files">More info on Sending Files »</a></td>
     * </tr>
     * <tr>
     * <td>duration</td>
     * <td>Integer</td>
     ...
     * <td>String</td>
     * <td>Optional</td>
     * <td>Send <a href="#markdown-style"><em>Markdown</em></a> or <a href="#html-style"><em>HTML</em></a>, if you want Core apps to show <a href="#formatting-* options">bold, italic, fixed-width text or inline URLs</a> in the media caption.</td>
     * </tr>
     * <tr>
     * <td>disable_notification</td>
     * <td>Boolean</td>
     ...
     * <td><a href="https://core.telegram.org/bots/api#inputfile">InputFile</a> or String</td>
     * <td>Yes</td>
     * <td>Sticker to send. You can either pass a <em>file_id</em> as String to resend a sticker that is already on the Core servers, or upload a new sticker using <strong>multipart/form-data</strong>.</td>
     * </tr>
     * <tr>
     * <td>reply_to_message_id</td>
     * <td>Integer</td>
    ...
    /**
     * Use this method to send video files, Core clients support mp4 videos (other formats may be sent as Document). On success, the sent Message is returned. Bots can currently send video files of up to 50 MB in size, this limit may be changed in the future.<br/>Values inside $content:<br/>
     * <table>
     * <tr>
     * <td><strong>Parameters</strong></td>
     * <td><strong>Type</strong></td>
     ...
     * <td><a href="https://core.telegram.org/bots/api#inputfile">InputFile</a> or String</td>
     * <td>Yes</td>
     * <td>Video to send. Pass a file_id as String to send a video that exists on the Core servers (recommended), pass an HTTP URL as a String for Core to get a video from the Internet, or upload a new video using <strong>multipart/form-data</strong>.</td>
     * </tr>
     * <tr>
     * <td>duration</td>
     * <td>Integer</td>
    ...
    /**
     *  Use this method to send audio files, if you want Core clients to display the file as a playable voice message. For this to work, your audio must be in an .ogg file encoded with OPUS (other formats may be sent as Audio or Document). On success, the sent Message is returned. Bots can currently send voice messages of up to 50 MB in size, this limit may be changed in the future.<br/>Values inside $content:<br/>
     * <table>
     * <tr>
     * <td><strong>Parameters</strong></td>
     * <td><strong>Type</strong></td>
     ...
     * <td><a href="https://core.telegram.org/bots/api#inputfile">InputFile</a> or String</td>
     * <td>Yes</td>
     * <td>Audio file to send. Pass a file_id as String to send a file that exists on the Core servers (recommended), pass an HTTP URL as a String for Core to get a file from the Internet, or upload a new one using <strong>multipart/form-data</strong>.</td>
     * </tr>
     * <tr>
     * <td>caption</td>
     * <td>String</td>
    ...
    /**
     *  Use this method when you need to tell the user that something is happening on the bot's side. The status is set for 5 seconds or less (when a message arrives from your bot, Core clients clear its typing status).
     *
    ...
    /**
     *  Use this method to to download a file from the Core servers.
     * \param $telegram_file_path String File path on Core servers
     ...
     *
     * If you'd like to make sure that the Webhook request comes from Core, we recommend using a secret path in the URL, e.g. https://www.example.com/<token>. Since nobody else knows your botâ€˜s token, you can be pretty sure itâ€™s us.
    ...
    public function Username()
    {
        ...
        if ($type == self::CHANNEL_POST) {
            return @$this->data['channel_post']['from']['username'] ?? null;
        }
        if ($type == self::EDITED_MESSAGE) {
            return @$this->data['edited_message']['from']['username'] ?? null;
        }
    ...
    /** Upon receiving a message with this object, Core clients will hide the current custom keyboard and display the default letter-keyboard. By default, custom keyboards are displayed until a new keyboard is sent by a bot. An exception is made for one-time keyboards that are hidden immediately after the user presses a button.
     * \param $selective Boolean Use this parameter if you want to show the keyboard to specific users only. Targets: 1) users that are @mentioned in the text of the Message object; 2) if the bot's message is a reply (has reply_to_message_id), sender of the original message.
    ...
    /* Upon receiving a message with this object, Core clients will display a reply interface to the user (act as if the user has selected the bot‘s message and tapped ’Reply'). This can be extremely useful if you want to create user-friendly step-by-step interfaces without having to sacrifice privacy mode.
     * \param $selective Boolean Use this parameter if you want to show the keyboard to specific users only. Targets: 1) users that are @mentioned in the text of the Message object; 2) if the bot's message is a reply (has reply_to_message_id), sender of the original message.
     ...
     * <td>String</td>
     * <td>Optional</td>
     * <td>Required if <em>ok</em> is False. Error message in human readable form that explains why it is impossible to complete the order (e.g. "Sorry, delivery to your desired address is unavailable'). Core will display this message to the user.</td>
     * </tr>
     * </table>
     * \param $content the request parameters as array
     * \return the JSON Core's reply.
    ...
    /**
     * As of <a href="https://telegram.org/blog/video-messages-and-telescope">v.4.0</a>, Core clients support rounded square mp4 videos of up to 1 minute long. Use this method to send video messages. On success, the sent <a href="https://core.telegram.org/bots/api#message">Message</a> is returned.
     * <table>
     * <tr>
     * <td><strong>Parameters</strong></td>
     * <td><strong>Type</strong></td>
     ...
     * <td><a href="https://core.telegram.org/bots/api#inputfile">InputFile</a> or String</td>
     * <td>Yes</td>
     * <td>Video note to send. Pass a file_id as String to send a video note that exists on the Core servers (recommended) or upload a new video using multipart/form-data. <a href="https://core.telegram.org/bots/api#sending-files">More info on Sending Files »</a>. Sending video notes by a URL is currently unsupported</td>
     * </tr>
     * <tr>
     * <td>duration</td>
     * <td>Integer</td>
     ...
     * <td><a href="https://core.telegram.org/bots/api#inputfile">InputFile</a> or String</td>
     * <td>Yes</td>
     * <td>Photo to send. Pass a file_id as String to send a photo that exists on the Core servers (recommended), pass an HTTP URL as a String for Core to get a photo from the Internet, or upload a new photo using multipart/form-data. <a href="https://core.telegram.org/bots/api#sending-files">More info on Sending Files »</a></td>
     * </tr>
     * <tr>
     * <td>caption</td>
     * <td>String</td>
     ...
     * <td><a href="https://core.telegram.org/bots/api#inputfile">InputFile</a> or String</td>
     * <td>Yes</td>
     * <td><strong>Png</strong> image with the sticker, must be up to 512 kilobytes in size, dimensions must not exceed 512px, and either width or height must be exactly 512px. Pass a <em>file_id</em> as a String to send a file that already exists on the Core servers, pass an HTTP URL as a String for Core to get a file from the Internet, or upload a new one using multipart/form-data. <a href="https://core.telegram.org/bots/api#sending-files">More info on Sending Files »</a></td>
     * </tr>
     * <tr>
     * <td>emojis</td>
     * <td>String</td>
